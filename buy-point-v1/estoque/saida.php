...
if(!$p||$q<=0||($q > (float)$p['estoque']))throw new RuntimeException('Quantidade inválida ou estoque insuficiente.');
$pdo->prepare("UPDATE produtos SET estoque=estoque-? WHERE id=?")->execute([$q,$pid]);
$pdo->prepare("INSERT INTO movimentacoes_estoque(produto_id,usuario_id,tipo,quantidade,observacao) VALUES(?,?,?,?,?)")->execute([$pid,$_SESSION['usuario_id'],'SAIDA',$q,$obs]);
$pdo->commit();header('Location:index.php');exit;}catch(Throwable $e){if($pdo->inTransaction())$pdo->rollBack();$erro=$e->getMessage();}}
?>
<!doctype html><html lang="pt-BR"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Saída de estoque</title><link rel="stylesheet" href="../assets/css/style.css"></head><body><?php include __DIR__.'/../partials/menu.php';?><main class="main"><h1>Saída de estoque</h1><?php if($erro):?><div class="alert danger"><?=htmlspecialchars($erro)?></div><?php endif;?><div class="panel"><form method="post">
...
<div class="actions"><a class="btn" href="index.php">Cancelar</a><button class="btn danger-btn">Registrar saída</button></div></form></div></main></body></html>